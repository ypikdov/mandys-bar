...
import type { PaginatedResponse, Reservation, ConfirmReservationPayload } from '@mandys/shared';
 ...
 */
export async function createClientReservation(payload: any, token: string | null) {
 ...
 */
export async function getAllReservations(token: string, options?: ListQueryOptions): Promise<PaginatedResponse<Reservation>> {
  const query = options ? `?${buildListQueryString(options)}` : '';
  const response = await apiGet(`/api/reservations${query}`, token);
  if (!response.ok) {
    throw new Error('Error obteniendo reservaciones');
  }
  return normalizePaginatedResponse((await response.json()) as PaginatedResponse<Reservation> | Reservation[]);
}
