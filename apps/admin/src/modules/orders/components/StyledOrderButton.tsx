...
const StyledOrderButton = ({ onClick, disabled, isSent }: StyledOrderButtonProps) => {
  return (
    <div className="styled-button-wrapper">
      <button 
        type="button"
        className={`order-button ${isSent ? 'sent' : ''} ${disabled && !isSent ? 'is-animating' : ''}`} 
        onClick={onClick} 
        disabled={disabled}
      >
        <div className="button-outline" />
        <div className="button-state state--default">
          <div className="button-icon">
            <svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <g className="button-shadow-filter">
          ...
          <p>
            {"Registrar y Confirmar".split("").map((char, i) => (
              <span key={i} style={{ '--i': i } as CSSProperties}>{char === " " ? "\u00A0" : char}</span>
            ))}
          </p>
        </div>
        <div className="button-state state--sent">
          <div className="button-icon">
          ...
          <p>
            {"¡Pedido Enviado!".split("").map((char, i) => (
              <span key={i} style={{ '--i': i + 5 } as CSSProperties}>{char === " " ? "\u00A0" : char}</span>
            ))}
          </p>
        </div>
      </button>
