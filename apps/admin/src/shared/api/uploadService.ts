 ...
 *
 * Centraliza la subida de imágenes (productos, perfil, contenido web).
...
export type UploadImageBucket = 'product-images' | 'gallery' | 'events' | 'users-avatars';
...
export async function uploadImage(
  ...
  const response = await apiUpload(`/api/upload?bucket=${encodeURIComponent(bucket)}`, formData, token);
  if (!response.ok) {
